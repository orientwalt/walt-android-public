...
public class Sign {
    ...
    public static String recoverSigningAddress(RawTransaction tx, SignatureData signatureData)
        ...
        byte[] r = signatureData.getR();
        byte[] s = signatureData.getS();
        SignatureData signatureDataV = new SignatureData(getRealV(v), r, s);
        BigInteger key = signedMessageToKey(encodedTransaction, signatureDataV);
        return NEOSign.Companion.getAddress(key);
    }
