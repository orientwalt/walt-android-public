...
public class UpdateDialogFragment extends DialogFragment implements View.OnClickListener {
    ...
    //默认色
    private int mDefaultColor = 0xff317CFB;
    private int mDefaultPicResId = R.mipmap.app_top_bg;
...
//        getDialog().getWindow().addFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL);
    ...
    private void initView(View view) {
        ...
        //关闭按钮+线 的整个布局
        rl_close = view.findViewById(R.id.iv_close);
        //顶部图片
    ...
    private void initData() {
        ...
        if (mUpdateApp != null) {
            ...
            if (!TextUtils.isEmpty(targetSize)) {
                msg = String.format(getResources().getString(R.string.update_size), targetSize);
            }
            ...
            //标题
            mTitleTextView.setText(TextUtils.isEmpty(dialogTitle) ? String.format("V%s", newVersion) : dialogTitle);
            //强制更新
...
//        mUpdateOkButton.setBackgroundDrawable(DrawableUtil.getDrawable(AppUpdateUtils.dip2px(4, getActivity()), color));
    ...
    private void install() {
        ...
        if (flag != PackageManager.PERMISSION_GRANTED) {
            if (ActivityCompat.shouldShowRequestPermissionRationale(getActivity(), Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                ...
                Toast.makeText(getActivity(), getResources().getString(R.string.update_tig), Toast.LENGTH_LONG).show();
            } else {
                // 申请授权。
    ...
    public void onClick(View view) {
        int i = view.getId();
        if (i == R.id.btn_ok) {
            if (SysUtils.getNetWorkState(mActivity) == 1) {
                //权限判断是否有访问外部存储空间权限
                install();
            } else {
                new CBDialogBuilder(mActivity)
                        ...
                        .setMessage(getResources().getString(R.string.update_dialog_msg))
                        .setConfirmButtonText(getResources().getString(R.string.update_dialog_commit))
                        .setCancelButtonText(getResources().getString(R.string.update_dialog_cancel))
...
//            }
    ...
    public void cancelDownloadService() {
        if (mDownloadBinder != null) {
            // 标识用户已经点击了更新，之后点击取消
            mDownloadBinder.stop(getResources().getString(R.string.update_cancel_load));
        }
    }

    private void installApp() {
        if (UpdateVersionServiece.INSTANCE.getPath().equals(mUpdateApp.getApkFileUrl())) {
        ...
        if (mUpdateApp != null) {
            ...
            binder.start(mUpdateApp, new DownloadService.DownloadCallback() {
                ...
                public boolean onFinish(final File file) {
                    if (!UpdateDialogFragment.this.isRemoving()) {
                        UpdateVersionServiece.INSTANCE.setPath(mUpdateApp.getApkFileUrl());
                        if (mUpdateApp.isConstraint()) {
                            showInstallBtn(file);
                        } else {
